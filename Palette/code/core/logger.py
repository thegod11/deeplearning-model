...
class InfoLogger():
    ...
    def __getattr__(self, name): # 在使用逗号调用不存在属性时
...
class VisualWriter():
    ...
    def __init__(self, opt, logger):
        ...
        if enabled and self.rank==0:   # 分布式训练，只在rank = 0上打印信息（只在rank=0上初始化writer）
            log_dir = str(log_dir)
    ...
    def save_CT_images(self, results):
        result_path = os.path.join(self.result_dir, self.phase)
        os.makedirs(result_path, exist_ok=True)
        result_path = os.path.join(result_path, str(self.epoch))
        os.makedirs(result_path, exist_ok=True)
        ct_np_result_path = os.path.join(result_path, "ct_np")

        ''' get names and corresponding images from results[OrderedDict] '''
        try:
            names = results['name']
            outputs = Util.postprocess4CT(results['result'])
            for i in range(len(names)): 
                # Image.fromarray(outputs[i]).save(os.path.join(result_path, names[i]))
                np.save(os.path.join(ct_np_result_path, names[i]), outputs[i])
        except:
            raise NotImplementedError('You must specify the context of name and result in save_current_results functions of model.')
