...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ...
        self.load_networks()
        # filter 过滤模型中需要训练的参数传给adam，设置了lr与weight_decay
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            ret_path.append('Compare_Input_GT_Out_{}'.format(self.path[idx]))
            input = self.cond_image[idx].detach()[:].float().cpu()
            gt = self.gt_image[idx].detach().float().cpu()
            out = self.visuals[idx-self.batch_size].detach().float().cpu()
            ret_result.append(torch.cat((torch.cat((input, gt), dim=2), out), dim=2))
        
        if self.task in ['inpainting','uncropping']:
            ret_path.extend(['Mask_{}'.format(name) for name in self.path])
            ret_result.extend(self.mask_image)
        # collections.namedtuple('CustomResult', 'name result')
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                for key, value in self.train_metrics.result().items():  # self.train_metrics.result()用来整理数据成日志格式
                    self.logger.info('{:5s}: {}\t'.format(str(key), value))
                    self.writer.add_scalar(key, value)
            if self.iter % (self.opt['train']['log_iter'] * 10) == 0:
                with torch.no_grad():
                    if self.opt['distributed']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num)
                    else:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num)
                for key, value in self.get_current_visuals(phase='train_val').items():
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                for met in self.metrics:  # metrics是评测函数，val_metrics是log，writer是tensorboard
                    key = met.__name__
                ...
                for key, value in self.get_current_visuals(phase='val').items():
                    self.writer.add_images(key, value)
                self.writer.save_images(self.save_current_results())  # 每个batch保存一次
